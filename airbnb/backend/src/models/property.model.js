...
async function listProperties({ city, country, min_price, max_price, type, sort, page = 1, page_size = 20, viewer_id = null } = {}) {
  const params = [];
  let sql = `
    SELECT
      p.id,
...
async function countProperties({ city, country, min_price, max_price, type } = {}) {
  const params = [];
