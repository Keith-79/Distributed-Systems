...
export default function SearchBar(){
  ...
  return (
    <div className="sticky top-[64px] z-10" ref={wrapRef}>
      ...
      {open==='who' && (
        <div className="mx-auto max-w-[900px]">
          <div className="mt-2 bg-white rounded-2xl shadow-xl border p-4">
            ...
            <div className="mt-3 flex justify-end"><button className="px-3 py-1 rounded-full border" onClick={()=>setOpen(null)}>Done</button></div>
          </div>
        </div>
      )}
      ...
      <Modal isOpen={showCal} onClose={()=>setShowCal(false)}>
        <div className="max-w-3xl">
          <div className="flex items-center justify-between mb-3">
            ...
            {/* clear button that auto-applies and stays open */}
            <button aria-label="Clear dates" title="Clear dates" className="px-3 py-1 rounded-full border text-sm" onClick={()=>{ setStartDate(''); setEndDate(''); }}>Clear</button>
          </div>
          <AirbnbStyleRangePicker
            ...
            onClear={()=>{ /* local clear only, do not apply */ }}
          />
