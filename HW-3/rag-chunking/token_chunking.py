...
def summarize_vec(vec, n_show=8):
    ...
    return {
        ...
        "mean": float(vec.mean()),
        "std": float(vec.std()),
        "first_dims": [float(x) for x in vec[:n_show]]
    }
...
def main():
    ...
    splitter = TokenTextSplitter(
        ...
        tokenizer=enc.encode  # callable returning list[int]
    )
    nodes = splitter.get_nodes_from_documents([Document(text=text)])
    print(f"[chunking] TokenTextSplitter -> nodes={len(nodes)} (size={args.chunk_size}, overlap={args.overlap})")

    # 3) embedding model
    emb = HuggingFaceEmbedding(model_name="sentence-transformers/all-MiniLM-L6-v2")
    dim_test = len(emb.get_text_embedding("dimension probe"))
    print(f"[emb] model=all-MiniLM-L6-v2 dim={dim_test}")

    # 4) show a couple of chunk embeddings (stats + first dims)
    ...
    index = faiss.IndexFlatIP(dim_test)
    ...
    # 6) retrieval (no LLM; retrieval-only)
