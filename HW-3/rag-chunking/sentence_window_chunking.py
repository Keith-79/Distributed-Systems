...
def main():
    ...
    parser.add_argument("--window-size", type=int, default=2, help="# prev/next sentences to include")
    ...
    # NOTE: removed include_prev_next_relations (no longer supported)
    parser_sw = SentenceWindowNodeParser.from_defaults(
        window_size=args.window_size,
        window_metadata_key="window"
    )
    nodes = parser_sw.get_nodes_from_documents([Document(text=text)])
    print(f"[chunking] SentenceWindow -> nodes={len(nodes)} (window_size={args.window_size})")
